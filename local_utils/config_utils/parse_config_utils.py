...
synth_chinese_cfg = Config(config_path='./config/synth_chinese_dataset.yaml')
synth_90k_cfg = Config(config_path='./config/synth_90k_dataset.yaml')
...
if __name__ == '__main__':
    ...
    test
    """
    train_epoch_nums = synth_chinese_cfg.TRAIN.EPOCH_NUMS
    batch_size = synth_chinese_cfg.TRAIN.BATCH_SIZE
    model_save_dir = synth_chinese_cfg.TRAIN.MODEL_SAVE_DIR
    snapshot_epoch = synth_chinese_cfg.TRAIN.SNAPSHOT_EPOCH
    enable_miou = synth_chinese_cfg.TRAIN.COMPUTE_MIOU.ENABLE
    with open('./test_config_file.json', 'w', encoding='utf-8') as file:
        synth_chinese_cfg.dump_to_json_file(file)
